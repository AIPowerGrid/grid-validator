...
- Production Core implements capabilities, worker discovery, assignments,
  targeted text probes, attestations, scorecards, assignment health, durable
  completed-result recovery, privacy-safe operator qualification, and
  non-economic shared 3-of-5 quorum states through migration `0026`.
  Independent operator control is not proven by registration alone.
...
video scorers. Production Core contains a separately gated, default-off
...
Core does not issue that weaker single-candidate lane. Its dark
